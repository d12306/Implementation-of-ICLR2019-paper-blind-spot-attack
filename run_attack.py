...
def run_attack(checkpoint, x_adv, epsilon):
  ...
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      ...
      cur_corr, y_pred_batch, prediction = sess.run([model.num_correct, model.y_pred, model.correct_prediction],
                                        feed_dict=dict_adv)
      ...
      if ibatch == 0:
        ...
        prediction_all = prediction_all.reshape((eval_batch_size,1))
        print(prediction_all.shape)

      else:
        prediction_all = np.concatenate((prediction_all, np.asarray(prediction).reshape((len(prediction),1))), axis =0 )
  ...
  print('Accuracy: {:.6f}%'.format(100.0 * accuracy))
